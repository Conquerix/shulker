...
if [ "$#" -ne 8 ]; then
	echo "usage: $0 <sync-script> <host-docs> <wrong-name> <mismatched-report> <noncanonical-report> <escaping-report> <relative-report> <line-break-report>" >&2
	exit 2
fi
...
escaping_host_docs="$6"
relative_host_docs="$7"
line_break_host_docs="$8"
