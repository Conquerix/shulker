...
'''
    ...
    for mode in ['bad-mount', 'inactive', 'snapshot-fails', 'restart-fails', 'ok']:
        ...
        p = subprocess.run(['bash', str(runtime), 'backup', str(root), 'pool/actual', str(root / 'lock'),
        ...
        if mode == 'restart-fails':
            assert 'zfs destroy pool/actual@borgmatic' in log, log
        if mode == 'ok':
            assert log.index('compose stop server') < log.index('zfs snapshot') < log.index('compose up'), log
    print('Actual backup: ordering, mount refusal, inactive refusal, snapshot and restart failure recovery passed')
