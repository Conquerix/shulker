    ...
    pkgs.runCommand "wiki-docs-contract"
      ...
      ''
        ...
        do
          ...
          if grep -Fqx -- "$publisher_marker" "$path" >/dev/null; then
            echo "$source: publisher marker" >&2
            exit 1
          fi
          ...
          if ! awk '
            /^(```|~~~)/ { fenced = !fenced; next }
            !fenced && /^# [^#]/ { headings += 1 }
            END { exit headings == 1 ? 0 : 1 }
          ' "$path"; then
            echo "$source: exactly one H1 outside fenced code blocks" >&2
            exit 1
          fi
          ...
          ' "$path"; then
            echo "$source: canonical Markdown targets" >&2
            exit 1
          fi
        done
