...
with lib;
let
  cfg = config.shulker.modules.shell.git;
...
in
{
  options.shulker.modules.shell.git = {
  ...
  config = mkIf cfg.enable {
      ...
      let
        ...
        extra =
          if !cfg.minimal then [
            ...
            (mkIf config.shulker.modules.shell.gnupg.enable gitAndTools.git-crypt)
          ] else
            [ ];
    ...
    shulker.modules.shell.bash.profileExtra = ''
