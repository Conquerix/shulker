...
with lib;
let
  cfg = config.shulker.system.modules.pterodactyl.panel;

  # Command to create a new user in the panel:
  #   docker exec -it pterodactyl_panel php artisan p:user:make
in
{
  options.shulker.system.modules.pterodactyl.panel = {
  ...
  config = mkIf cfg.enable {
    ...
    environment.persistence."/nix/persist".directories =
      mkIf (config.shulker.system.modules.impermanence.enable)
        [
          "${cfg.stateDir}"
        ];
  };
