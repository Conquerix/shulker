#!/usr/bin/env bash
# Serialize startup, shutdown and consistent snapshots of all Actual data.
set -euo pipefail
...
start)
	...
	"$compose" stop server
	if [ -d "$state_dir/data/server-files" ]; then
...
backup)
	...
	if [ "$("$compose" ps --status running --services)" != server ]; then
		echo 'Actual is not running; refusing to archive stale data' >&2
		exit 65
	fi
...
cleanup)
	...
	;;
*)
	echo 'Usage: actual-budget-runtime start|stop|backup|cleanup' >&2
	exit 64
	;;
esac
