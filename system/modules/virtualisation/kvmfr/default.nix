...
with lib;
let
  cfg = config.shulker.system.modules.virtualisation.kvmfr;
in
{
  options.shulker.system.modules.virtualisation.kvmfr = {
  ...
  config = mkIf cfg.enable {
    boot.extraModulePackages = with config.boot.kernelPackages; [
      (pkgs.callPackage ./git-package.nix { inherit kernel; })
    ];
